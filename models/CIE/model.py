...
class Net(CNN):
    ...
    def forward(self, data_dict, **kwargs):
        if 'images' in data_dict:
            ...
            UF_src = normalize_over_channels(src)
            UF_tgt = normalize_over_channels(tgt)
        ...
        if 'features' in data_dict:
            emb1, emb2 = torch.transpose(UF_src, 1, 2), torch.transpose(UF_tgt, 1, 2)
        else: # U_src, F_src are features at different scales

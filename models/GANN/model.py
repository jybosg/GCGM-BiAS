...
class Net(CNN):
    ...
    def real_forward(self, data_dict, num_clusters, **kwargs):
        ...
        # * non-vision data
            ...
            # determine max number of features
            max_features = max(t.shape[1] for t in feats)
            # adjust tensor shapes to fit pad_tensor function
            adjusted_feats = [t.squeeze(0).permute(1, 0) for t in feats]  # [no_of_features, feature_dim]
            # pad the adjusted tensors
            padded_adjusted_feats = pad_tensor(adjusted_feats)
            # adjust the padded tensors back to original shape
            padded_feats = [t.permute(1, 0).unsqueeze(0) for t in padded_adjusted_feats]  # [1, feature_dim, no_of_features]
            # concatenate the padded tensors
            feats = torch.cat(padded_feats, dim=0)
            feats = normalize_over_channels(feats)
            feats[torch.isnan(feats)] = 0.
        ...
        ms = torch.zeros(batch_size, num_graphs, dtype=torch.int, device=feats[0].device)
        ...
        A = [torch.zeros(m.item(), m.item(), device=feats[0].device) for m in mssum]
        for idx, feat, P, As_src, n in feat_list:
            ...
            # get non-zero edge lengths
            non_zero_edge_lens = edge_lens[edge_lens != 0]
            # handle case where all edge lengths are zero
            if non_zero_edge_lens.nelement() == 0:
                median_lens = torch.zeros(1, device=non_zero_edge_lens.device)
            else:
                median_lens = torch.median(non_zero_edge_lens)
        ...
        Wds = [torch.zeros(m.item(), m.item(), device=feats[0].device) for m in mssum]
        ...
        for b in range(batch_size):
            ...
            U0_b = torch.full((torch.sum(ms[b]), univ_size), 1 / univ_size.to(dtype=torch.float), device=feats[0].device)

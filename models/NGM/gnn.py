...
class GNNLayer(nn.Module):
    ...
    #             nn.init.trunc_normal_(module.weight.data, mean=0.0, std=1.0)
    #             if module.bias is not None:
    #                 nn.init.constant_(module.bias.data, 0)

    def forward(self, A, W, x, n1=None, n2=None, n1_max=None, n2_max=None, norm=True):
    # def forward(self, A, W, x, n1=None, n2=None, norm=True):
        ...
        if self.classifier is not None:
            if n1_max is None:
                ...
                n1_rep = torch.repeat_interleave(n1, self.sk_channel, dim=0)
                n2_rep = torch.repeat_interleave(n2, self.sk_channel, dim=0)
                x4 = x3.permute(0,2,1).reshape(x.shape[0] * self.sk_channel, n2_max, n1_max).transpose(1, 2)
                x5 = self.sk(x4, n1_rep, n2_rep, dummy_row=True).transpose(2, 1).contiguous()

                x6 = x5.reshape(x.shape[0], self.sk_channel, n1_max * n2_max).permute(0, 2, 1)
                x_new = torch.cat((x2, x6), dim=-1)
        else:
            x_new = x2

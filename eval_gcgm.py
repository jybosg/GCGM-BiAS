...
def eval_model(model, dataloader, classes, verbose=False, xls_sheet=None):
    ...
    for i, cls in enumerate(classes):
        ...
        for inputs in dataloader:
            ...
            if cfg.PROBLEM.TYPE in ['2GM', 'GCL', 'GCLTS', 'PRETRAIN']:
                ...
                if 'aff_mat' in outputs:
                    ...
                    normalized = torch.zeros_like(pred_obj_score)
                    mask = (pred_obj_score * gt_obj_score != 0)
                    normalized[mask] = pred_obj_score[mask] / gt_obj_score[mask]
                    objs[i] += torch.sum(normalized)
    ...
    if not torch.any(torch.isnan(objs)) or not torch.all(objs.eq(0)):
    ...
    return torch.Tensor(list(map(torch.mean, precisions))), torch.Tensor(list(map(torch.mean, recalls))), torch.Tensor(list(map(torch.mean, f1s))), objs

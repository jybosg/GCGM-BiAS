...
def matching_f1(pmat_pred: Tensor, pmat_gt: Tensor, ns: Tensor) -> Tensor:
    ...
    f1 = torch.zeros(batch_num, device=device)
    ...
    for b in range(batch_num):
        ...
        # acc[b] = torch.sum(pmat_pred[b] * pmat_gt[b]) / torch.sum(pmat_gt[b])
        # precision[b] = torch.sum(pmat_pred[b] * pmat_gt[b]) / torch.sum(pmat_pred[b])
    
    acc[torch.isnan(acc)] = 1
    precision[torch.isnan(precision)] = 1
    mask = (acc * precision != 0)
    f1[mask] = 2 * acc[mask] * precision[mask] / (acc[mask] + precision[mask])
    return f1

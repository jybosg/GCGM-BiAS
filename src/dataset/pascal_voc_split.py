...
class PascalVOCSplit:
    def __init__(self, sets, rate_1, rate_2, obj_resize):
        """
        :param sets: 'train' or 'test'
        :param obj_resize: resized object size
        """
        self.classes = cfg.PascalVOCSplit.CLASSES
        self.kpt_len = [len(KPT_NAMES[_]) for _ in cfg.PascalVOCSplit.CLASSES]

        anno_path = cfg.PascalVOCSplit.KPT_ANNO_DIR
        img_path = cfg.PascalVOCSplit.ROOT_DIR + 'JPEGImages'
        ori_anno_path = cfg.PascalVOCSplit.ROOT_DIR + 'Annotations'
        set_path = cfg.PascalVOCSplit.SET_SPLIT
        ...
        assert self.sets in ('train', 'validation', 'test'), 'No match found for dataset {}'.format(self.sets)
        if self.sets in ['train', 'validation']:
            cache_name = 'voc_db_' + 'train' + '.pkl'
        else:
            cache_name = 'voc_db_' + 'test' + '.pkl'
        ...
        if self.cache_file.exists():
            ...
            #         num = int(len(self.xml_list[cls_id]) * self.rate_1)
            #         if num < 2:
            #             num = 2
            #         # split train and validation
            #         if self.rate_1 > 0.5:
            #             random.seed(cfg.RANDOM_SEED)
            #             random.shuffle(self.xml_list[cls_id])
            #             self.xml_list[cls_id] = self.xml_list[cls_id][0: num]
            #         else:
            #             random.seed(cfg.RANDOM_SEED)
            #             random.shuffle(self.xml_list[cls_id])
            #             self.xml_list[cls_id] = self.xml_list[cls_id][-num:]
            ...
            #     train_xml_list, val_xml_list = [], []
            #     for cls_list in self.xml_list:
            #         train_subset, val_subset = train_test_split(cls_list, test_size=self.val_split_size, random_state=cfg.RANDOM_SEED)
            #         train_xml_list.append(train_subset)
            #         val_xml_list.append(val_subset)
            #     self.xml_list = train_xml_list if sets == "train" else val_xml_list
            ...
            if self.sets in ['train', 'validation']:
                for cls_id in range(len(self.classes)):
                    ...
                    if self.val_split_size > 0:
                        train_subset, val_subset = train_test_split(self.xml_list[cls_id], test_size=self.val_split_size, random_state=cfg.RANDOM_SEED)
                        self.xml_list[cls_id] = train_subset if sets == "train" else val_subset
        else:
            print('Caching xml list to {}...'.format(self.cache_file))
            self.cache_path.mkdir(exist_ok=True, parents=True)
            with np.load(set_path, allow_pickle=True) as f:
                self.xml_list = f[self.sets]
    ...
    def get_ssl_pair(self, cls=None, shuffle=True, tgt_outlier=True, src_outlier=True):
        ...
        if cls is None:
            ...
            cls = random.choice([self.classes.index(c) for c in cls])
...
if __name__ == '__main__':
    dataset = PascalVOCSplit('train', (256, 256))
    a = dataset.get_pair()
    pass

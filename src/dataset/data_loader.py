...
from src.build_graphs import build_graphs, prep_graphs
...
class GMDataset(Dataset):
    def __init__(self, name, length, rate_1, rate_2, cls=None, problem='2GM', **args):
        self.name = name
        # if not any(x in self.name for x in ['Synthetic', 'AIDS']):
        ...
        self.cls = None if cls in ['none', 'all'] else cls.split(', ')
        ...
        if self.cls is None:
            ...
            if not isinstance(self.cls, list):
    ...
    def __getitem__(self, idx):
        if self.problem_type in ['2GM', 'GCL', 'GCLTS', 'PRETRAIN']:
    ...
    @staticmethod
    def to_pyg_graph(A, P, index=None, x=None):
        ...
        # * add dummy_label to the nodes for recognizing corresponding nodes from two corrupted views
        pyg_graph = pyg.data.Data(
            x=torch.tensor(P / rescale).to(torch.float32) if x is None else x.to(torch.float32),
            ...
            dummy_label=torch.unsqueeze(torch.arange(A.shape[0]), 1) if index is None else torch.unsqueeze(index, 1),
            # * corrdinates of the nodes
            pos=torch.tensor(P).to(torch.float32)
        )
        return pyg_graph
    ...
    def get_pair(self, idx, cls):
        ...
        # * add support for different GM settings
        if cfg.PROBLEM.FILTER in ['intersection', 'inclusion', 'unfiltered']:
            try:
                anno_pair, perm_mat = self.ds.get_pair(cls, problem=self.problem_type, tgt_outlier=cfg.PROBLEM.TGT_OUTLIER, src_outlier=cfg.PROBLEM.SRC_OUTLIER)
            except TypeError:
                anno_pair, perm_mat = self.ds.get_pair(cls, problem=self.problem_type)
            ...
            if cfg.PROBLEM.FILTER == 'subset':
                while any([min(perm_mat.shape[0], perm_mat.shape[1]) <= 2, perm_mat.size >= cfg.PROBLEM.MAX_PROB_SIZE > 0, len(anno_pair[0]['keypoints']) == len(anno_pair[1]['keypoints'])]):
                    anno_pair, perm_mat = self.ds.get_pair(cls, problem=self.problem_type, tgt_outlier=cfg.PROBLEM.TGT_OUTLIER, src_outlier=cfg.PROBLEM.SRC_OUTLIER)
            elif cfg.PROBLEM.FILTER == 'difference':
                while any([min(perm_mat.shape[0], perm_mat.shape[1]) <= 2, perm_mat.size >= cfg.PROBLEM.MAX_PROB_SIZE > 0, 
                           set([key['name'] for key in anno_pair[0]['keypoints']]).issubset(set([key['name'] for key in anno_pair[1]['keypoints']])),
                           set([key['name'] for key in anno_pair[0]['keypoints']]).issuperset(set([key['name'] for key in anno_pair[1]['keypoints']]))]):
                    anno_pair, perm_mat = self.ds.get_pair(cls, problem=self.problem_type, tgt_outlier=cfg.PROBLEM.TGT_OUTLIER, src_outlier=cfg.PROBLEM.SRC_OUTLIER)
        ...
        if anno_pair[0].get('A') is not None:
            A1, G1, H1, e1 = prep_graphs(anno_pair[0]['A'], n1, sym=cfg.GRAPH.SYM_ADJACENCY)
            A2, G2, H2, e2 = prep_graphs(anno_pair[1]['A'], n2, sym=cfg.GRAPH.SYM_ADJACENCY)
            pyg_graph1 = self.to_pyg_graph(A1, P1, x=anno_pair[0]['x'])
            pyg_graph2 = self.to_pyg_graph(A2, P2, anno_pair[1].get('index'), anno_pair[1]['x'])
        else:
            ...
            pyg_graph2 = self.to_pyg_graph(A2, P2, anno_pair[1].get('index'))
...
def collate_fn(data: list):
    ...
    if 'Gs' in ret and 'Hs' in ret:
        if cfg.PROBLEM.TYPE in ['2GM', 'GCL', 'GCLTS', 'PRETRAIN'] and len(ret['Gs']) == 2 and len(ret['Hs']) == 2:
            G1, G2 = ret['Gs']
            H1, H2 = ret['Hs']
            if cfg.FP16:
    ...
    # print('data_loader - pyg_graphs: {}'.format(ret['pyg_graphs']))

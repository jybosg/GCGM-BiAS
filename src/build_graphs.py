...
# A = G @ H.T
def build_graphs(P_np: np.ndarray, n: int, n_pad: int=None, edge_pad: int=None, stg: str='tri', sym: bool=True,
...
# * prep graph if A was given
def prep_graphs(A: np.ndarray, n: int, n_pad: int=None, edge_pad: int=None, sym: bool=True):
...
# the edge feature is the concatenation of the two ending nodes
...
def reshape_edge_feature_diff(F: Tensor, G: Tensor, H: Tensor, device=None) -> Tensor:
    """
    Given point-level features extracted from images, compute the difference of node features for each edge
    based on the order of G, H.

    :param F: (b x d x n) extracted point-level feature matrix.
              b: batch size. d: feature dimension. n: number of nodes.
    :param G: (b x n x e) factorized adjacency matrix, where A = G * H^T. e: number of edges.
    :param H: (b x n x e) factorized adjacency matrix, where A = G * H^T
    :param device: device. If not specified, it will be the same as the input
    :return: edge feature matrix X (b x d x e)
    """
    if device is None:
        device = F.device
    ...
    # Compute node features associated with each edge's source and target
    source_features = torch.matmul(F, G)
    target_features = torch.matmul(F, H)

    # Compute the difference between node features for each edge
    X = source_features - target_features

...
# * default lr configurations
__C.TRAIN.FINETUNE_BACKBONE = True
__C.TRAIN.SEPARATE_BACKBONE_LR = False
__C.TRAIN.BACKBONE_LR = 1.e-5

__C.TRAIN.FINETUNE_ENCODER = True
__C.TRAIN.SEPARATE_ENCODER_LR = False
__C.TRAIN.ENCODER_LR = 1.e-4
...
def get_output_dir(model, dataset, config_name=None):
    """
    Return the directory where experimental artifacts are placed.
    :param model: model name
    :param dataset: dataset name
    ...
    """
    if config_name:
        outp_path = os.path.join('output', '{}_{}_{}'.format(model, dataset, config_name))
    else:
        outp_path = os.path.join('output', '{}_{}'.format(model, dataset))
    return outp_path
...
def _merge_a_into_b(a, b):
    ...
    for k, v in a.items():
        ...
        if k not in b:
            ...
            # * escape for hyperparameter configurations
            b[k] = v
            continue

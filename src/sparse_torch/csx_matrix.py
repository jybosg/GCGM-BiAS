...
def _max(inp, *args, **kwargs):
    if type(inp) == np.ndarray:
        ...
        raise ValueError('Data type {} not understood.'.format(type(inp)))
# ! address the input was an empty tensor
# def _max(inp, *args, **kwargs):
#     if type(inp) == np.ndarray:
#         if inp.size == 0:  # If the numpy array is empty
#             return np.nan  # Return a default value
#         return np.max(inp, *args, **kwargs)
#     elif type(inp) == torch.Tensor:
#         if inp.numel() == 0:  # If the tensor is empty
#             return torch.tensor(float('nan'))  # Return a default value
#         return torch.max(inp, *args, **kwargs)
#     else:
#         raise ValueError('Data type {} not understood.'.format(type(inp)))
